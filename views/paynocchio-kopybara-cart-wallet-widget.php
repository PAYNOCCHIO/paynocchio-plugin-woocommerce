...
<div class="paynocchio-cart-wallet-widget cfps-flex cfps-flex-row cfps-items-center cfps-relative cfps-text-black">
    <div class="cart cfps-bg-white cfps-rounded-lg cfps-p-2">
        <a href="/paynocchio-account/" alt="Checkout" title="Checkout">
            <div class="cfps-flex cfps-flex-row cfps-items-center">
                <img src="<?php echo plugin_dir_url( WOOCOMMERCE_PAYNOCCHIO_BASENAME ) . 'assets/img/profile_black.png' ?>" class="!cfps-h-[25px] cfps-w-auto cfps-mr-2"/>
                    <p class="cfps-font-semibold">Account</p>
            </div>
        </a>
    </div>
    ...
    <?php if (is_user_logged_in() && get_user_meta(get_current_user_id(), PAYNOCCHIO_WALLET_KEY)) {
        ...
        ?>
        <div class="wallet paynocchio-wallet <?php if ($wallet['status'] !== 'ACTIVE') { ?>cfps-disabled<?php } ?> cfps-bg-white cfps-rounded-lg cfps-p-2">
